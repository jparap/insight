...
type VaultServiceSpec struct {
	...
	// ETCD Version of etcd to be deployed.
	// Default: 3.2.13.
	EtcdVersion string `json:"etcdVersion"`

	// clusterDomain used for certs.
	// Default: cluster.local.
	ClusterDomain string `json:"clusterDomain"`

	// AutoUnseal to unseal the cluster. if true cluster will auto unseal
	// unseal tokens stored as k8s secrets in cluster namespace.
	// Default: false.
	AutoUnseal bool `json:"autoUnseal,omitempty"`

	// SanHosts custom hostnames to auto generated ssl cets
	//TODO: implement letsencrypt/cert-manager...
	SanHosts []string `json:"sanHosts,omitempty"`
...
func (v *VaultService) SetDefaults() bool {
	...
	if len(vs.EtcdVersion) == 0 {
		vs.EtcdVersion = defaultEtcdVersion
		changed = true
	}
	if len(vs.ClusterDomain) == 0 {
		vs.ClusterDomain = defaultClusterDomain
		changed = true
	}
	// if vs.AutoUnseal == false {
	// 	vs.AutoUnseal = false
	// } else {
	// 	vs.AutoUnseal = true
	// }
