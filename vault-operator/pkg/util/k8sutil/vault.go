...
func DeployEtcdCluster(etcdCRCli etcdCRClient.Interface, v *api.VaultService) error {
	...
	etcdCluster := &etcdCRAPI.EtcdCluster{
		...
		ObjectMeta: metav1.ObjectMeta{
			...
			//Annotations: map[string]string{"etcd.database.coreos.com/scope": "clusterwide"},
		},
		Spec: etcdCRAPI.ClusterSpec{
			Size:    size,
			Version: v.Spec.EtcdVersion,
	...
	if v.Spec.EtcdAnnotations != nil {
		etcdCluster.ObjectMeta.Annotations = v.Spec.EtcdAnnotations
	}
	if v.Spec.Pod != nil {
...
// configVaultClientTLS mounts the volume containing the vault server TLS assets for the vault pod
