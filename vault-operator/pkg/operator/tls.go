...
var (
	...
	//defaultClusterDomain = api.VaultService.Spec.ClusterDomain
)
...
func newEtcdServerTLSSecret(vr *api.VaultService, caKey *rsa.PrivateKey, caCrt *x509.Certificate) (*v1.Secret, error) {
	return newTLSSecret(vr, caKey, caCrt, "etcd server", k8sutil.EtcdServerTLSSecretName(vr.Name),
		[]string{
			...
			fmt.Sprintf("*.%s.%s.svc.%s", k8sutil.EtcdNameForVault(vr.Name), vr.Namespace, vr.Spec.ClusterDomain),
			fmt.Sprintf("%s-client.%s.svc.%s", k8sutil.EtcdNameForVault(vr.Name), vr.Namespace, vr.Spec.ClusterDomain),
		},
		map[string]string{
			"key":  "server.key",
			"cert": "server.crt",
...
func newVaultServerTLSSecret(vr *api.VaultService, caKey *rsa.PrivateKey, caCrt *x509.Certificate) (*v1.Secret, error) {
	return newTLSSecret(vr, caKey, caCrt, "vault server", api.DefaultVaultServerTLSSecretName(vr.Name),
		append([]string{
			"localhost",
			"127.0.0.1",
			fmt.Sprintf("%s", vr.Name),
			fmt.Sprintf("%s.%s", vr.Name, vr.Namespace),
			fmt.Sprintf("*.%s.pod", vr.Namespace),
			fmt.Sprintf("%s.%s.svc", vr.Name, vr.Namespace),
			fmt.Sprintf("*.%s.%s.svc", vr.Name, vr.Namespace),
			fmt.Sprintf("%s.%s.svc.%s", vr.Name, vr.Namespace, vr.Spec.ClusterDomain),
			fmt.Sprintf("*.%s.%s.svc.%s", vr.Name, vr.Namespace, vr.Spec.ClusterDomain),
			fmt.Sprintf("*.%s", vr.Spec.ClusterDomain),
		}, vr.Spec.SanHosts...),
		map[string]string{
