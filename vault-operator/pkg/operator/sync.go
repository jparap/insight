...
func (v *Vaults) handleErr(err error, key interface{}) {
	...
	if v.queue.NumRequeues(key) < maxRetries {
		logging.Errorf("error syncing Vault (%v): %v", key, err)
	...
	logging.Infof("Dropping Vault (%v) out of the queue: %v", key, err)
}
...
func (v *Vaults) syncVault(key string) (err error) {
	...
	if !exists {
		logging.Infof("Vault CR (%s) is deleted", key)
		return nil
	}
...
func (v *Vaults) reconcileVault(vr *api.VaultService) (err error) {
	...
	if vr.Status.Phase == api.ClusterPhaseInitial {
		...
		logging.Infof("creating etcd cluster: %s/%s", vr.Namespace, k8sutil.EtcdNameForVault(vr.Name))
	...
	// TODO: make use of deployment informer, may have to go first
